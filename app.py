...
from lib.story_engine import story_engine
...
async def readiness_probe(
    ...
    """Readiness probe for orchestrators (returns 503 until a pipeline is loaded)."""
    ready = pipeline_manager is not None and pipeline_manager.is_loaded()
    payload = {
...
class StoryCueRequest(BaseModel):
    cue: str
...
@app.get("/api/v1/story/themes")
async def list_story_themes():
    """Return the available LongLive channel presets."""
    return {"themes": story_engine.list_themes()}


@app.post("/api/v1/story/session", response_model=StoryStateResponse)
async def start_story_session(request: StorySessionRequest):
    """Initialize a new narrative session for the selected theme."""
    try:
        payload = await story_engine.start_session(request.theme_id)
        return StoryStateResponse(active=True, **payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:  # pragma: no cover - defensive logging
        logger.error(f"Error starting story session: {exc}")
        raise HTTPException(status_code=500, detail="Failed to start story session") from exc


@app.post("/api/v1/story/cue", response_model=StoryStateResponse)
async def submit_story_cue(request: StoryCueRequest):
    """Advance the current story using the provided cue."""
    try:
        payload = await story_engine.submit_cue(request.cue)
        return StoryStateResponse(active=True, **payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:  # pragma: no cover - defensive logging
        logger.error(f"Error updating story: {exc}")
        raise HTTPException(status_code=500, detail="Failed to update story") from exc


@app.get("/api/v1/story/state", response_model=StoryStateResponse)
async def fetch_story_state():
    """Return the currently active story session, if any."""
    payload = story_engine.get_state()
    if not payload:
        return StoryStateResponse(active=False)
    return StoryStateResponse(active=True, **payload)
