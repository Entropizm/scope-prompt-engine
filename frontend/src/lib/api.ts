...
export interface StoryStatePayload {
  active: boolean;
  theme: StoryTheme | null;
  story_text: string;
  prompt_log: StoryLogEntry[];
  cues: string[];
...
export const fetchStoryThemes = async (): Promise<StoryThemesResponse> => {
  const response = await fetch("/api/v1/story/themes", {
    method: "GET",
    headers: { "Content-Type": "application/json" },
  });
  if (!response.ok) {
...
export const startStorySession = async (
  themeId: string
): Promise<StoryStatePayload> => {
  const response = await fetch("/api/v1/story/session", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `Failed to start story: ${response.status} ${response.statusText}: ${errorText}`
    );
  }
  return response.json();
};

export const sendStoryCue = async (
  cue: string
): Promise<StoryStatePayload> => {
  const response = await fetch("/api/v1/story/cue", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export const fetchStoryState = async (): Promise<StoryStatePayload> => {
  const response = await fetch("/api/v1/story/state", {
    method: "GET",
    headers: { "Content-Type": "application/json" },
  });
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `Failed to fetch story state: ${response.status} ${response.statusText}: ${errorText}`
    );
  }
  return response.json();
};
